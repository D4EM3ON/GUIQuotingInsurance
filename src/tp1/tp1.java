...
public class tp1 extends javax.swing.JFrame {
    ...
    /**
     * Permet d'initialiser le JFrame
     */
    public tp1() {
        ...
        infoParticulieresLayout.setHorizontalGroup(
            ...
            .addGroup(infoParticulieresLayout.createSequentialGroup()
                ...
                .addGroup(infoParticulieresLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addComponent(labelTitre2, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                    .addGroup(infoParticulieresLayout.createSequentialGroup()
                        .addGroup(infoParticulieresLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addGroup(infoParticulieresLayout.createSequentialGroup()
    ...
    /**
     * Regarde si toutes les entrées sont remplies, sinon le dit et les met en rouge.
     * Si tout est beau, crée un conducteur et le set avec les infos données. Passe ensuite au prochain card, c'est à dire les infos particulières
     * @param evt n'est pas utilisé ici
     */
    private void boutonProchain1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_boutonProchain1ActionPerformed
        ...
        if (this.inputEmail.getText().matches(EMAIL_PATTERN) && !inputBirthday.getText().isBlank() && !inputNom.getText().isBlank() && !inputNumeroCivique.getText().isBlank() && inputPhone.getText().matches(PHONE_PATTERN)&& !inputPostal.getText().isBlank() && !inputPrenom.getText().isBlank() && !inputRue.getText().isBlank() && !inputVille.getText().isBlank()){
            ...
            if (inputBirthday.getText().isBlank()) inputBirthday.setBorder(BorderFactory.createLineBorder(red)); 
            if (inputNom.getText().isBlank()) inputNom.setBorder(BorderFactory.createLineBorder(red)); 
            if (inputNumeroCivique.getText().isBlank()) inputNumeroCivique.setBorder(BorderFactory.createLineBorder(red)); 
            if (!inputPhone.getText().matches(PHONE_PATTERN)) inputPhone.setBorder(BorderFactory.createLineBorder(red)); 
            if (inputPrenom.getText().isBlank()) inputPrenom.setBorder(BorderFactory.createLineBorder(red)); 
            if (inputPostal.getText().isBlank()) inputPostal.setBorder(BorderFactory.createLineBorder(red)); 
            if (inputRue.getText().isBlank()) inputRue.setBorder(BorderFactory.createLineBorder(red));
            if (inputVille.getText().isBlank()) inputVille.setBorder(BorderFactory.createLineBorder(red));
    ...
    /**
     * Lorsque le focus est mis sur un text field, remet son border et sa grandeur à ce qu'il était originalement
     * @param evt permet d'aller chercher d'où viens le focus
     */
    ...
    /**
     * Regarde si les informations sont bien remplies dans la 2e fenêtre. Si oui, passe au prochain et passe les infos au conducteur. Sinon, dit à l'utilisateur de bien remplir sa soumission
     * @param evt n'est pas utilisé ici
     */    
    ...
    /**
     * Si jamais le checkbox autre est cliqué dessus, regarde si c'est on/off, et active ou désactive en conséquence le text box autre pour remplir la condition autre
     * @param evt ne fait rien dans ce cas ci
     */
    ...
    /**
     * Regarde si tout est bien rempli, calcule la prime, l'affiche au conducteur et l'amène au fichier.
     * @param evt n'est pas utilisé ici
     */
    private void boutonCalculActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_boutonCalculActionPerformed
        ...
        if (this.inputYear.getText().isBlank() || Integer.parseInt(this.inputYear.getText()) > 2024 || Integer.parseInt(this.inputYear.getText()) < 1886) {this.inputYear.setBorder(BorderFactory.createLineBorder(red)); yearTrue = true;}
        
        try {
            Integer.parseInt(this.inputKm.getText());
        } catch (Exception e){
            kmTrue = true;
        }
        if (this.inputKm.getText().isBlank() || kmTrue || Integer.parseInt(this.inputKm.getText()) < 0) {this.inputKm.setBorder(BorderFactory.createLineBorder(red)); kmTrue = true;}
        ...
        if (this.checkAutre.isSelected() && this.inputAutre.getText().isBlank()) { this.inputAutre.setBorder(BorderFactory.createLineBorder(red)); autreTrue = true; }
    ...
    /**
     * Lorsque le checkbox Aucun est cliqué dessus, active les autres s'il devient faux, les désactive s'il devient vrai. À chaque fois, les uncheck tous. 
     * @param evt n'est pas utilisé ici
     */
    ...
    /**
     * Met la couleur du texte pour habitation à la normale lorsque le radio button Oui dessous gain focus
     * @param evt n'est pas utilisé ici
     */
    ...
    /**
     * Met la couleur du texte pour habitation à la normale lorsque le radio button Non dessous gain focus
     * @param evt n'est pas utilisé ici
     */
    ...
    /**
     * Lorsqu'une marque de voiture est choisie, modifie le combobox modèles pour refléter les différents modèles de la marque.
     * @param evt n'est pas utilisé ici
     */
    ...
    /**
     * Lorsqu'un checkbox a le focus sur lui dans la 3e page, pour les antivols, permet de remettre la couleur du texte du label qui vient avec à sa couleur originale
     * @param evt n'est pas utilisé ici
     */
    ...
    /**
     * Lorsqu'un combobox a le focus sur lui dans la 3e page, pour les marques/modèles, permet de remettre la couleur de son border à sa couleur originale
     * @param evt permet d'aller chercher si c'est soit le combobox de la marque, ou du modèle et d'aller modifier correspondant
     */
    ...
    /**
     * Permet de regarder si tout est beau au niveau du courriel lorsque l'utilisateur passe à autre chose. si oui, il le remet à sa couleur originale. 
     * Sinon, il met la couleur du courriel entré en rouge
     * @param evt n'est pas utilisé dans ce cas ci
     */
    ...
    /**
     * Lorsque l'année du véhicule est rentrée, regarde si elle est bonne ou non. Si oui, remet la couleur du texte à sa couleur originale, sinon le met en rouge
     * @param evt n'est pas utilisé ici
     */
    private void inputYearFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_inputYearFocusLost
        if (!this.inputYear.getText().isBlank() && (Integer.parseInt(this.inputYear.getText()) > 2024 || Integer.parseInt(this.inputYear.getText()) < 1886)) {
    ...
    /**
     * Regarde si la date de fête est bonne. Si oui, met la couleur du texte à sa couleur originale. Sinon, la met en rouge
     */
    private void inputBirthdayFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_inputBirthdayFocusLost
        ...
        int birthdayYear = Integer.parseInt(this.inputBirthday.getText().substring(0, 4));
        
        if (!DATE_PATTERN.matcher(this.inputBirthday.getText()).matches() || birthdayYear >= todaysYear || birthdayYear <= 1900) {
    ...
    /**
     * Permet de mettre la couleur du texte du input de km si jamais elle est mauvaise ou bonne
     * @param evt n'est pas utilisé ici
     */
    private void inputKmFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_inputKmFocusLost
        try {
            if (!this.inputKm.getText().isBlank() && Integer.parseInt(this.inputKm.getText()) < 0) this.inputKm.setForeground(this.red);
            else this.inputKm.setForeground(new JTextField().getForeground());
        } catch (Exception e){
            this.inputKm.setForeground(new JTextField().getForeground());
        }
        
    }//GEN-LAST:event_inputKmFocusLost
